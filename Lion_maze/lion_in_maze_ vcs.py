...
def search_algo(n, maze, start, end):
    ...
    pq = []
    heapq.heapify(pq)
    heapq.heappush(pq, (0, pos ,0))
    # {cost till this node , node position , cost incurred in last step to reach this node}
    while pos != end and element_count > 0:

        cost, pos, cost_1 = heapq.heappop(pq)
        element_count -= 1
        ...
        if pos % n != 0 and maze[(pos - 1) // n][(pos - 1) % n] == 0:
            heapq.heappush(pq, (cost + 2, pos - 1, 2))
            parent[pos - 1] = pos
            element_count += 1
        if pos - n >= 0 and maze[(pos - n) // n][(pos - n) % n] == 0:
            heapq.heappush(pq, (cost + 2, pos - n, 2))
            parent[pos - n] = pos
            element_count += 1
        if pos + n < n * n and maze[(pos + n) // n][(pos + n) % n] == 0:
            heapq.heappush(pq, (cost + 3, pos + n, 3))
            parent[pos + n] = pos
            element_count += 1
