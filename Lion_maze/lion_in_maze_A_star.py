...
def search_algo(n, maze, start, end):
    ...
    # A star search
    # heuristic values
    heuristic = [-1 for j in range(n * n)]
    for i in range(n*n):
        heuristic[i] = abs(i//n - (n*n-1)//n) + abs(i%n - (n*n-1)%n)
    ...
    pq = []
    heapq.heapify(pq)
    heapq.heappush(pq, (heuristic[pos]+0, 0, pos, 0))
    # ( h(n)+g(n) , g(n) , pos  , cost incurred in last step to reach this node)
    while pos != end and element_count > 0:

        cost_sm, cost, pos, cost_1 = heapq.heappop(pq)
        element_count -= 1
        ...
        if pos % n != 0 and maze[(pos - 1) // n][(pos - 1) % n] == 0:
            heapq.heappush(pq, (heuristic[pos-1] + cost + 2, cost + 2, pos - 1, 2))
            parent[pos - 1] = pos
            element_count += 1
        # above the current cell
        if pos - n >= 0 and maze[(pos - n) // n][(pos - n) % n] == 0:
            heapq.heappush(pq, (heuristic[pos - n] + cost + 2, cost + 2, pos - n, 2))
            parent[pos - n] = pos
            element_count += 1
        # below the current cell
        if pos + n < n * n and maze[(pos + n) // n][(pos + n) % n] == 0:
            heapq.heappush(pq, (heuristic[pos + n] + cost + 3, cost + 3, pos + n, 3))
            parent[pos + n] = pos
            element_count += 1
...
if __name__ == "__main__":
    ...
    actions, total_cost = search_algo(n, maze, start, end)
